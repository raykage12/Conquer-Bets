import random
import statistics
from v3fetch_players import PlayersMinutes
...
class Calculations:
    ...
    def calculate_summary_stats(self, player_points):
        if not player_points:
            raise ValueError("Player has no points data.")

        average_points = sum(player_points) / len(player_points)
        median_points = statistics.median(player_points)
        min_points = min(player_points)
        max_points = max(player_points)

        if len(player_points) > 1:
            std_dev_points = statistics.stdev(player_points)
        else:
            std_dev_points = 0.0
    ...
    def analyze_player(self, player, season, regular_season, spread, odds_over, odds_under):
        ...
        summary_stats = self.calculate_summary_stats(player_points)
        ...
        return {
            ...
            "points": player_points,
            "average_points": summary_stats["average_points"],
            "median_points": summary_stats["median_points"],
            "min_points": summary_stats["min_points"],
            "max_points": summary_stats["max_points"],
            "std_dev_points": summary_stats["std_dev_points"],
            "p_over": p_over,
            "p_under": p_under,
            "over_ci": (lower, upper),
    ...
    def format_results(self, results):
        ...
        recent_points = results["points"][:10]
        formatted_points = ", ".join(str(point) for point in recent_points)

        lines = [
            "-" * 27 + f" {results['player'].upper()} " + "-" * 27,
            f"Games: {results['games']} | Recent Points: {formatted_points}",
            (
                f"Avg: {results['average_points']:.2f} | "
                f"Median: {results['median_points']:.2f} | "
                f"Min/Max: {results['min_points']}/{results['max_points']} | "
                f"Std Dev: {results['std_dev_points']:.2f}"
            ),
            (
